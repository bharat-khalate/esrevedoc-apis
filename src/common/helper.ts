 ...
 */
export const toLowerCaseEnum = <T extends Record<string, string>>(
  enumObject: T,
  ...
  ) as { [K in keyof T]: Lowercase<T[K]> };
};

export const normalizeSearchFields = (search: any) => {
 ...
 */
export const getSearchFilterPrismaCondition = (
  searchFields: TSearchFields,
  requestFields: Record<string, TSearchValue | undefined>,
) => {
...
const buildPrismaSearchCondition = (
...
const buildPrismaRegexCondition = (
  config: TSearchConfig,
  value: TSearchValue,
...
const buildPrismaEqualCondition = (
  config: TSearchConfig,
  value: TSearchValue,
): Record<string, unknown> | null => {
  const values = config.allowMulti && Array.isArray(value) ? value : [value];
...
const buildPrismaRangeCondition = (
  field: string,
  value: TSearchValue,
): Record<string, unknown> | null => {
...
const buildPrismaDateCondition = (
  ...
  if (config.dataType === "range" && values.length === 2) {
    const start = new Date(values[0] as string);
    const end = new Date(values[1] as string);
    end.setHours(23, 59, 59, 999);
    return { [config.field]: { gte: start, lte: end } };
  }

  return { [config.field]: new Date(values[0] as string) };
};

const parseSearchDate = (value: string): string | null => {
